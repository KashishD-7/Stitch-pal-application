...
class _ShopDetailsScreenState extends State<ShopDetailsScreen> {
  ...
  Future<void> _fetchShopDetails() async {
    ...
    try {
      ...
      // Only proceed if user is a Tailor
      if (userType != 'Tailor') {
        setState(() {
          _errorMessage = 'This page is only available for Tailor accounts';
          _isLoading = false;
        });
        return;
      }
      ...
      final response = await http.get(
        Uri.parse('http://172.16.6.10:8000/api/shop-details/'),
        headers: {
  ...
  Future<void> _updateShopDetails() async {
    ...
    try {
      SharedPreferences prefs = await SharedPreferences.getInstance();
      String? token = prefs.getString('token');
      String apiUrl = "http://172.16.6.10:8000/api/update-shop/";
                                    ...
                                    child: !kIsWeb && _imageFile != null
                                      ...
                                      errorBuilder: (context, error, stackTrace) {
                                        return Container(
                                          ...
                                          child: Center(
                                            child: Icon(Icons.store, size: isSmallScreen ? 60 : 80, color: Colors.grey),
                                          ),
                                        );
                                      },
